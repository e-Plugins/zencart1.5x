...
// Digiwallet Gateway
$objDigiCore = new DigiwalletCore($method, $transaction_info->fields['rtlo'], 'nl', $testMode);
...
if($realstatus == "open")
{
        ...
        if($method == "BW"){
            ...
            if (!empty($consumber_info) && $consumber_info['bw_paid_amount'] > 0) {
                if ($consumber_info['bw_paid_amount'] < $consumber_info['bw_due_amount']) {
                    ...
                    $order_success_status = constant('MODULE_PAYMENT_DIGIWALLET' . $pay_type . 'ORDER_STATUS_ID_REVIEW');
                }
            }
        }
    } else {
    ...
    if($method == "AFP"){
        ...
        if (strtolower($status) == "captured") {
            ...
            // Show error message
            $messageStack->add_session('checkout_payment', constant("MODULE_PAYMENT_DIGIWALLET{$pay_type}ERROR_TEXT_ERROR_OCCURRED_PROCESSING") . "<br/>" . $reject_reason);
