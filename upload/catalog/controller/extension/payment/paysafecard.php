...
class ControllerExtensionPaymentPaysafecard extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->load->view($this->config->get('config_template') . 'extension/payment/paysafecard.tpl', $data);
    }
    ...
    public function send()
    {
        ...
        $rtlo = ($this->config->get('paysafecard_rtlo')) ? $this->config->get('paysafecard_rtlo') : TargetPayCore::DEFAULT_RTLO; // Default Digiwallet
    ...
    {
        $sql = "INSERT INTO `" . DB_PREFIX . TargetPayCore::TARGETPAY_PREFIX . TargetPayCore::METHOD_PAYSAFE . "` SET "
                . "`order_id`='" . $this->db->escape($order_id)
                . "', " . "`method`='" . $this->db->escape($method)
                . "', `" . TargetPayCore::METHOD_PAYSAFE . "_txid`='" . $this->db->escape($txid) . "'";
        $this->db->query($sql);
    }
}
