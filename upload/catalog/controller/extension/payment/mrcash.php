...
class ControllerExtensionPaymentMrcash extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->load->view($this->config->get('config_template') . 'extension/payment/mrcash.tpl', $data);
    }
    ...
    public function send()
    {
        ...
        $rtlo = ($this->config->get('mrcash_rtlo')) ? $this->config->get('mrcash_rtlo') : TargetPayCore::DEFAULT_RTLO; // Default Digiwallet
    ...
    {
        $sql = "INSERT INTO `" . DB_PREFIX . TargetPayCore::TARGETPAY_PREFIX . TargetPayCore::METHOD_MRCASH ."` SET " .
                "`order_id`='" . $this->db->escape($order_id) . "', " .
                "`method`='" . $this->db->escape($method) . "', " .
                "`" . TargetPayCore::METHOD_MRCASH . "_txid`='" . $this->db->escape($txid) . "'";
        $this->db->query($sql);
    }
}
