...
class ControllerExtensionPaymentTPCallback extends Controller
{
     ...
     *
     * example usage: $digiCore->setReportUrl(
    ...
    public function report()
    {
        ...
        if (! empty($this->request->get["order_id"])) {
            $order_id = (int) $this->request->get["order_id"]; // Buggy redirects
        }
        
        if (empty($order_id) || empty($payment_type)) {
            $this->log->write('Digiwallet tp_callback(), no order_id passed');
            echo "NoOrderId ";
            die();
        }
        ...
        $digiCoreTx = !empty($trxid) ? $this->getTxid($order_id, $trxid, $var_type) : null;
        ...
        $rtlo = ($this->config->get($var_type . '_rtlo')) ? $this->config->get($var_type . '_rtlo') : TargetPayCore::DEFAULT_RTLO; // Default Digiwallet
        
        $digiCore = new TargetPayCore($payment_type, $rtlo, TargetPayCore::APP_ID, "nl", false);
        $digiCore->checkPayment($digiCoreTx[$var_type . "_txid"]);
        
        if ($digiCore->getPaidStatus() || $this->config->get($var_type . '_test')) {
