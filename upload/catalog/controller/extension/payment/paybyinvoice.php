...
class ControllerExtensionPaymentPaybyinvoice extends Controller
{
    ...
    public function send()
    {
        ...
        $rtlo = ($this->config->get('paybyinvoice_rtlo')) ? $this->config->get('paybyinvoice_rtlo') : 93929; // Default Digiwallet
        ...
        if ($order_info['currency_code'] != "EUR") {
            ...
            $digiCore = new TargetPayCore ("AFT", $rtlo, "e59dbd219e068daade7139be42c5dfd5",
                $order_info['language_code'], false);
            $digiCore->setAmount(round($order_info['total'] * 100));
            $digiCore->setDescription("Order #" . $this->session->data['order_id']);

            $digiCore->setCancelUrl($this->url->link('checkout/cart', '', 'SSL'));
            $digiCore->setReturnUrl($this->url->link('checkout/success', '', 'SSL'));
            $digiCore->setReportUrl($this->url->link('extension/payment/paybyinvoice/callback',
                'order_id=' . $this->session->data['order_id'], 'SSL'));
            $digiCore->setCurrency($order_info['currency_code']);

            $digiCore->bindParam("cgender", "")// Resume
            ...
            $this->storeTxid($digiCore->getPayMethod(), $digiCore->getTransactionId(),
    ...
    public function callback()
    {
        ...
        if ($order_id == 0) {
            $this->log->write('Digiwallet callback(), no order_id passed');
            die();
        }
        ...
        $digiCoreTx = $this->getTxid($order_id, $_POST["trxid"]);

        if (!$digiCoreTx) {
            $this->log->write('Could not find Digiwallet transaction data for order_id=' . $order_id);
            die();
        }

        $rtlo = ($this->config->get('paybyinvoice_rtlo')) ? $this->config->get('paybyinvoice_rtlo') : 93929; // Default Digiwallet

        $digiCore = new TargetPayCore ("AFT", $rtlo, "e59dbd219e068daade7139be42c5dfd5", "nl", false);
        $digiCore->checkPayment($digiCoreTx["paybyinvoice_txid"]);
        ...
        if ($digiCore->getPaidStatus() || $this->config->get('paybyinvoice_test')) {
            ...
            echo "Not paid " . $digiCore->getErrorMessage() . "... ";
        }
