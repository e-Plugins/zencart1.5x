...
if (($_GET['targetpay_repair_order'] == 'true')&&(zen_targetpay_repair_allowed())) {
    $digiCore_repair_result = $db->Execute("SELECT transaction_id, order_id, ideal_session_data  FROM " . TABLE_TARGETPAY_TRANSACTIONS . " WHERE (transaction_id='" . zen_db_input($_GET['targetpay_transaction_id']) . "' AND `transaction_status` = 'success')");
