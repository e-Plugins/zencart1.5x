...
class digiwallet_a03_afp extends digiwalletBase
{
     ...
     * Contructor
     */
    public function digiwallet_a03_afp()
    {
        parent::digiwallet();
    }
    ...
    private static function format_phone($country, $phone) {
        ...
        if(method_exists('digiwallet_afp', $function)) {
        ...
        */
        ...
        if (strtolower($status) != "captured") {
            ...
            if (strtolower($status) == "rejected") {
                ...
                $messageStack->add_session('checkout_payment', $this->getConstant("MODULE_PAYMENT_DIGIWALLET{$this->payType}ERROR_TEXT_ERROR_OCCURRED_PROCESSING") . "<br/>" . $ext_info);
                zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode($this->getConstant("MODULE_PAYMENT_DIGIWALLET{$this->payType}ERROR_TEXT_ERROR_OCCURRED_PROCESSING") . " " . $ext_info), 'SSL', true, false));
                exit(0);
            } else {
