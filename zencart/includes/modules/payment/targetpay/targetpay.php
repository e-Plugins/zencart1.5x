...
class targetpayBase extends base
{
    ...
    {
        $this->code = 'targetpay_' . strtolower($this->tpPaymentMethodId);
        ...
        $this->title = $this->getConstant('MODULE_PAYMENT_TARGETPAY' . $this->payType . 'TEXT_TITLE');

        $this->description = $this->getConstant('MODULE_PAYMENT_TARGETPAY' . $this->payType . 'TEXT_DESCRIPTION') . $this->getConstant("MODULE_PAYMENT_TARGETPAY_TESTMODE_WARNING_MESSAGE");
        $this->sort_order = $this->getConstant('MODULE_PAYMENT_TARGETPAY' . $this->payType . 'SORT_ORDER');

        $this->enabled = (($this->getConstant('MODULE_PAYMENT_TARGETPAY' . $this->payType . 'STATUS') == 'True') ? true : false);

        $this->rtlo = $this->getConstant('MODULE_PAYMENT_TARGETPAY' . $this->payType . 'TARGETPAY_RTLO');

        $this->transactionDescription = $this->getConstant('MODULE_PAYMENT_TARGETPAY' . $this->payType . 'TRANSACTION_DESCRIPTION');
        $this->transactionDescriptionText = $this->getConstant("MODULE_PAYMENT_TARGETPAY" . $this->payType . "MERCHANT_TRANSACTION_DESCRIPTION_TEXT");

        $this->reportURL = zen_href_link('targetpay_callback.php?dw_action=callback', '', 'SSL', false, false, true);
        $this->returnURL = zen_href_link('targetpay_callback.php?dw_action=process', '', 'SSL', false, false, true);
        $this->cancelURL = zen_href_link('targetpay_callback.php?dw_action=cancel', '', 'SSL', false, false, true);
    ...
    public function update_status()
    {
        ...
        if (($this->enabled == true) && ((int) constant("MODULE_PAYMENT_TARGETPAY{$this->payType}ZONE") > 0)) {
            $check_flag = false;
            $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . constant("MODULE_PAYMENT_TARGETPAY{$this->payType}ZONE") . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    public function getDirectory()
    {
        ...
        $objDigiCore = new TargetPayCore($this->tpPaymentMethodId, $this->rtlo);
    ...
    public function check()
    {
        ...
        if (! isset($this->_check)) {
            $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "STATUS'");
    ...
    public function install()
    {
        ...
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Digiwallet payment module', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "STATUS', 'True', 'Do you want to accept Digiwallet payments?', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sortorder', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "SORT_ORDER', '0', 'Sort order of payment methods in list. Lowest is displayed first.', '6', '7', now())");
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Payment zone', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "ZONE', '0', 'If a zone is selected, enable this payment method for that zone only.', '6', '3', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");

        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Transaction description', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "TRANSACTION_DESCRIPTION', 'Automatic', 'Select automatic for product name as description, or manual to use the text you supply below.', '6', '8', 'zen_cfg_select_option(array(\'Automatic\',\'Manual\'), ', now())");

        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Transaction description text', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "MERCHANT_TRANSACTION_DESCRIPTION_TEXT', '" . TITLE . "', 'Description of transactions from this webshop. <strong>Should not be empty!</strong>.', '6', '9', now())");
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Digiwallet Outlet Identifier', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "TARGETPAY_RTLO', " . TargetPayCore::DEFAULT_RTLO . " , 'The Digiwallet layout code', '6', '2', now())"); // Default Digiwallet

        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('DigiWallet API Token', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "DIGIWALLET_API_TOKEN', '' , 'The API Token is used for new authentication model and refunding function', '6', '3', now())");

        /*
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Testaccount?', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "TESTACCOUNT', 'False', 'Enable testaccount (only for validation)?', '6', '6', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
        */

        //$db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('IP address', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "REPAIR_IP', '" . $_SERVER['REMOTE_ADDR'] . "', 'The IP address of the user (administrator) that is allowed to complete open ideal orders (if empty everyone will be allowed, which is not recommended!).', '6', '10', now())");
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable pre order emails', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "EMAIL_ORDER_INIT', 'False', 'Do you want emails to be sent to the store owner whenever an Digiwallet order is being initiated? The default is <strong>False</strong>.', '6', '17', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");

        $db->Execute("CREATE TABLE  IF NOT EXISTS " . TABLE_TARGETPAY_DIRECTORY . " (`issuer_id` VARCHAR( 4 ) NOT NULL ,`issuer_name` VARCHAR( 30 ) NOT NULL ,`issuer_issuerlist` VARCHAR( 5 ) NOT NULL ,`timestamp` DATETIME NOT NULL ,PRIMARY KEY ( `issuer_id` ) );");
        ...
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
            values ('Order status - confirmed', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "ORDER_STATUS_ID', '" . $confirmed . "', 'The status of orders that where successfully confirmed.', '6', '4', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
            values ('Order status - open', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "ORDER_STATUS_ID_OPEN', '" . $open . "', 'The status of orders of which payment still inprogress.', '6', '5', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
            values ('Order status - review', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "ORDER_STATUS_ID_REVIEW', '" . $review . "', 'The status of orders which paid by Bankwire.', '6', '6', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
        $db->Execute("insert IGNORE into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added)
            values ('Order status - refund', 'MODULE_PAYMENT_TARGETPAY" . $this->payType . "ORDER_STATUS_ID_REFUND', '" . $refund . "', 'The status of orders which have been refunded.', '6', '7', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())");
    }
    ...
    public function keys()
    {
        ...
        $out =  array(
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'STATUS',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'SORT_ORDER',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'ZONE',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'ORDER_STATUS_ID',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'TESTACCOUNT',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'ORDER_STATUS_ID_OPEN',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'ORDER_STATUS_ID_REVIEW',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'ORDER_STATUS_ID_REFUND',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'TARGETPAY_RTLO',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'DIGIWALLET_API_TOKEN',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'TRANSACTION_DESCRIPTION',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'MERCHANT_TRANSACTION_DESCRIPTION_TEXT',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'EMAIL_ORDER_INIT',
            'MODULE_PAYMENT_TARGETPAY' . $this->payType . 'REPAIR_IP'
        );
     ...
     * @param TargetPayCore $objDigiCore
     */
    protected function startTransaction($cc_amount, $cc_description, $cc_purchaseID, $objDigiCore = null)
    {
        ...
        */
        $objDigiCore = ($objDigiCore) ? $objDigiCore : new TargetPayCore($this->tpPaymentMethodId, $this->rtlo, $this->language, $testMode);
    ...
    function admin_notification($zf_order_id)
    {
        ...
        if(constant("MODULE_PAYMENT_TARGETPAY{$this->payType}DIGIWALLET_API_TOKEN")) {
            ...
            if(!empty($trans->fields['transaction_id'])){
                ...
                if($remain_refund > 0) {
                    ...
                    $output .= '<td><textarea name="refund_comment" style="height: 50px; width: 400px;" placeholder="'.MODULE_PAYMENT_TARGETPAY_REFUND_COMMENT_PLACE_HOLDER.'"></textarea></td>';
                    $output .= '</tr>';
                    $output .= '<tr>';
    ...
    {
        global $db;
        $result = $db->Execute("select * from " . TABLE_TARGETPAY_TRANSACTIONS . " where `order_id` = $order_id AND `transaction_status` = '$status'");
        return $result;
    }
    /**
    ...
    {
        global $db;
        $result = $db->Execute("select * from " . TABLE_TARGETPAY_REFUND . " where `order_id` = $order_id order by datetimestamp asc");
        return $result;
    }
    /**
    ...
    public function _doRefund($oID)
    {
        ...
        */
        $apiToken = constant("MODULE_PAYMENT_TARGETPAY{$this->payType}DIGIWALLET_API_TOKEN");
        // Get Order
        ...
        if(!empty($trans->fields['transaction_id'])){
            ...
            if($action_type == 'cancel'){
                ...
                $digiCore = new TargetPayCore($this->tpPaymentMethodId, $this->rtlo, $this->language, $testMode);
                if(!$testMode && !$digiCore->deleteRefund($trans->fields['transaction_id'], $apiToken)){
                    ...
                    $messageStack->add_session(MODULE_PAYMENT_TARGETPAY_DELETE_REFUND_SUCCESS, 'success');
                    // UPDATE REFUND TABLE FOR TRACKING
                    $db->Execute("update " . TABLE_TARGETPAY_REFUND . " set status = 'cancelled' where refund_id = '". zen_db_input($refundID) ."'");
                    ...
                    if($total_refunded <= 0) {
                        ...
                        $success_status = constant("MODULE_PAYMENT_TARGETPAY{$this->payType}ORDER_STATUS_ID");
                        if($success_status) {
                ...
                if($remain_refund < $amount) {
                    // Show error message
                    $messageStack->add_session(MODULE_PAYMENT_TARGETPAY_REFUND_ERROR_NOT_ENOUGH_AMOUNT, 'error');
                } else if(empty($message)){
                    $messageStack->add_session(MODULE_PAYMENT_TARGETPAY_REFUND_ERROR_NO_MESSAGE, 'error');
                }else if($amount <= 0){
                    $messageStack->add_session(MODULE_PAYMENT_TARGETPAY_REFUND_ERROR_AMOUNT, 'error');
                } else {
                    ...
                    if(!$testMode && !$digiCore->refundInvoice($refundData, $apiToken)){
                        ...
                        $messageStack->add_session(MODULE_PAYMENT_TARGETPAY_REFUND_SUCCESS, 'success');
                        $refund_id = $digiCore->getRefundID();
                        if($testMode) {
                            $refund_id = time();
                        }
                        // INSERT TO REFUND TABLE FOR LOGGING
                        $db->Execute("insert into " . TABLE_TARGETPAY_REFUND . "
                        ...
                        $refund_status = constant("MODULE_PAYMENT_TARGETPAY{$this->payType}ORDER_STATUS_ID_REFUND");
                        if($refund_status) {
        ...
        {
            $messageStack->add_session(MODULE_PAYMENT_TARGETPAY_REFUND_ERROR_TRANSACTION_NOT_FOUND, 'error');
        }
    }
}
